            ...
            warninp_t       wi)
{
    ...
    printStringNoNewline(&inp, "TPI Insertion in slab from zmin to zmax, when 0 full box is used");
    ir->tpizmin = get_ereal(&inp, "tpizmin", 0, wi);
    ir->tpizmax = get_ereal(&inp, "tpizmax", 0, wi);
    if (ir->tpizmin < 0.0) {
        gmx_fatal(FARGS, "Minimum z coordinate for insertion must be larger than zero");
    }
    if (ir->tpizmax < 0.0) {
        gmx_fatal(FARGS, "Maximum z coordinate for insertion must be larger than zero");
    }
